...
| 脚本 | 一句话解释 | 打个比方 |
|------|-----------|---------|
| `train_pretrain.py` | 让模型做"完形填空"，学语言规律 | 读万卷书，背成语词典 |
| `train_full_sft.py` | 教模型"怎么答问题" | 学完形填空之后，做"问答题答题技巧培训" |
| `train_lora.py` | 只学一点点，成本很低 | 买了一套西装但只改裤脚 |
| `train_distillation.py` | 让小学生抄大学生的作业 | 跟学霸一起做题，学霸的思路 |
| `train_dpo.py` | 告诉模型"这个答得好，那个答得差" | 老师批改作业：打勾或打叉 |
| `train_ppo.py` | 强化学习：教练+裁判一起教 | 请私教 + 陪练，边打边学 |
| `train_grpo.py` | 同桌互批：你比我好，我学学 | 同学互相打分，进步快的帮进步慢的 |
| `train_spo.py` | 一对一辅导：专门解决"都不行"的问题 | 私教单独盯着你补弱项 |
| `train_reason.py` | 训练模型输出"思考过程" | 教模型先打草稿再写答案 |
| `train_tokenizer.py` | 从文本里自动学哪些词该放词典 | 统计常用词组，做一本小词典 |
...
### 5.7 PPO（train_ppo.py）—— 经典强化学习教练

**核心思想**：PPO 是最经典的 RL 算法，需要训练两个模型——Actor（学生）和 Critic（裁判）。Actor 生成回答，Critic 估计每个 token 的价值，PPO 用裁剪（clip）防止一次更新变化太大。

**为什么叫"近端"（Proximal）？** 因为 PPO 限制每次更新步幅——新策略和旧策略的概率比不能超过 `[1-ε, 1+ε]`，防止策略剧烈变化导致性能崩溃。

**三模型架构**：
- **Actor（Policy）模型**：真正要训练的模型，负责生成回答
- **Old Actor 模型**：冻结的旧 Actor，用来计算策略变化比率
- **Critic（Value）模型**：评估"当前回答好不好打几分"，辅助 Actor 训练
- **Ref 模型**：参考模型，防止 Actor 跑偏太远（KL 散度约束）

### 5.8 SPO（train_spo.py）—— 单流策略优化（解决 GRPO 退化组问题）

**核心思想**：GRPO 需要对同一个问题生成 N 个回答组成"一群"来比较——但如果这 N 个回答质量都差（退化组），组内区分度为零，学习信号就消失了。

SPO 的解决方案：不用组内比较，而是用一个**自适应基线（AutoAdaptiveValueTracker）**来替代。基线根据历史奖励动态调整，持久化地跟踪奖励趋势，每个样本独立获得学习信号，不再依赖"同组伙伴"。

**ValueTracker 的工作方式**：维护一个 Beta 分布的参数（alpha, beta），根据最近一批奖励的均值，动态调整基线。KL 变化大时基线适应慢（保守），KL 变化小时基线适应快（激进）。

### 5.9 推理蒸馏（train_reason.py）—— 让模型学会"思考"

**核心思想**：训练模型输出类似 DeepSeek-R1 的推理格式：先输出 `<think>...\n</think>\n<answer>...\n</answer>`，即先"思考"再"回答"。

**关键技巧：格式惩罚**。代码对格式标记（`<think>`、`</think>`、`<answer>`、`</answer>`）所在位置的 loss 乘以 10 的惩罚系数：

```python
sp_ids = torch.isin(shift_labels, special_tokens)  # 格式标记位置
loss_mask_flat[sp_ids] = 10  # ×10 惩罚
```

这确保模型认真对待格式标记的位置，不会跳过思考直接给答案。

### 5.10 Tokenizer 训练（train_tokenizer.py）—— 制作自己的词典

**核心思想**：词表不是越大越好，也不是越小越好——需要找到压缩率和覆盖率的平衡点。

MiniMind 的 tokenizer 使用 BPE（Byte Pair Encoding）算法，从大量文本中自动学习哪些字符组合最常一起出现，把高频组合合并成一个 token。6400 个 token 对中文来说基本够用，压缩率约 1.5~1.7 字符/token。
